...
const sectionMenuButton = document.querySelector('.nav-sections');
const sectionMenu = document.querySelector('.section-menu');
const sectionMenuClose = document.querySelector('.section-menu-head button');

// Stable section anchors keep the expanded navigation useful without changing
// the approved content or visual order from the latest brief.
[
  ['.brand-ready', 'potential'],
  ['.distributor-value', 'advantages'],
  ['.v2-demand', 'demand'],
  ['.margins', 'commercial'],
  ['.app-ecosystem', 'smartcook'],
  ['.compliance', 'compliance'],
  ['.quality', 'quality'],
  ['.after-sales', 'after-sales'],
  ['.world', 'global'],
  ['.ip', 'ip'],
  ['.distributor-terms', 'terms'],
].forEach(([selector, id]) => {
...
sectionMenuButton?.addEventListener('click', () => {
  if (sectionMenu?.classList.contains('open')) closeSectionMenu();
  else openSectionMenu();
});
sectionMenuClose?.addEventListener('click', closeSectionMenu);
sectionMenu?.querySelectorAll('a').forEach((link) => link.addEventListener('click', () => {
  closeSectionMenu();
  closeMenu();
}));
document.addEventListener('pointerdown', (event) => {
  if (!sectionMenu?.classList.contains('open')) return;
  if (sectionMenu.contains(event.target) || sectionMenuButton?.contains(event.target)) return;
  closeSectionMenu();
});
...
document.addEventListener('keydown', (event) => { if (event.key === 'Escape') { closeMenu(); closeSectionMenu(); } });
...
// Anchor jumps and very fast touch scrolling can skip a native observer frame.
// This small viewport pass keeps editorial reveals reliable in both cases.
function revealInViewport() {
  document.querySelectorAll('.reveal:not(.visible), .global:not(.visible)').forEach((item) => {
    const box = item.getBoundingClientRect();
    if (box.top < window.innerHeight * .96 && box.bottom > 0) item.classList.add('visible');
  });
}
...
const editorialRails = [
  { track: '.brand-ready .four-cards', label: 'Highlights' },
  { track: '.benefit-grid', label: 'Advantages' },
  { track: '.manufacturing .process-grid', label: 'Process' },
  { track: '.innovation-points', label: 'Technology' },
  { track: '.ip-grid', label: 'Assets' },
  { track: '.terms-table', label: 'Terms' },
];
